...
This repo is the `phoenix_diagrams` library itself. `demo/` is a separate, full
...
- Add or update tests under `test/`. Loader fixtures live in
  `test/support/fixtures/phoenix_diagrams/` — add a new fixture subdirectory there
  rather than inline `.exs`/`.mmd` files in test bodies.
- If you touch the `.exs` diagram-definition format, that's `lib/phoenix_diagrams/loader.ex`.
...
- If you change `priv/static/phoenix_diagrams/` JS or CSS sources, rebuild the
  compiled output before committing:

  ```sh
  cd priv/static/phoenix_diagrams
  ...
  ```sh
  node --input-type=module --check < priv/static/phoenix_diagrams/build/bundle.js
  ```

  Only `priv/static/phoenix_diagrams/build/app.css` and
  `priv/static/phoenix_diagrams/build/bundle.js` are packaged/served at runtime — the
...
- **Zero host-asset coupling.** A host app must never need to edit its own
  `app.css`/`app.js` or add an npm dependency to use PhoenixDiagrams. Styles are
