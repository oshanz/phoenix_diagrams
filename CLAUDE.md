...
- `lib/ex_diag/diagram_live.ex` — the LiveView itself. On mount it calls `ExDiag.Loader.scan/1` against a configurable diagrams path (`Application.get_env(:ex_diag, :diagrams_path, ...)`, defaulting to `priv/ex_diag` under cwd) and renders a two-pane layout: a grouped sidebar of diagram entries and a detail pane that renders the selected entry's source via a `phx-hook` chosen from `entry.type` (`"ExDiagMermaid"` or `"ExDiagPlantuml"`, see `diagram_hook/1`) — `phx-update="ignore"` since the hook owns that DOM.
...
- `@plantuml/core`'s `viz-global.js` is a UMD/Emscripten build with an unreachable Node-only branch (`require("url")`) that esbuild still tries to statically resolve when bundled. Host apps' esbuild config needs `--external:url` (see `demo/config/config.exs`) — the branch never runs in the browser, so this is just a bundler quirk, not a real Node dependency.
